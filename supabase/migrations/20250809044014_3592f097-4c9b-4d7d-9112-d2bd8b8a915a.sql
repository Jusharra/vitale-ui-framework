-- Create marketplace tables for one-time payments (retry with guarded policy creation)
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'marketplace_pricing' 
      AND policyname = 'Public can view active marketplace pricing'
  ) THEN
    EXECUTE 'CREATE POLICY "Public can view active marketplace pricing" ON public.marketplace_pricing FOR SELECT USING (is_active = true)';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'marketplace_pricing' 
      AND policyname = 'Admins can manage marketplace pricing'
  ) THEN
    EXECUTE 'CREATE POLICY "Admins can manage marketplace pricing" ON public.marketplace_pricing FOR ALL USING ((auth.jwt() ->> ''role'') = ''admin'') WITH CHECK ((auth.jwt() ->> ''role'') = ''admin'')';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'marketplace_orders' 
      AND policyname = 'Admins can manage marketplace orders'
  ) THEN
    EXECUTE 'CREATE POLICY "Admins can manage marketplace orders" ON public.marketplace_orders FOR ALL USING ((auth.jwt() ->> ''role'') = ''admin'') WITH CHECK ((auth.jwt() ->> ''role'') = ''admin'')';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'marketplace_orders' 
      AND policyname = 'Users can view their own marketplace orders'
  ) THEN
    EXECUTE 'CREATE POLICY "Users can view their own marketplace orders" ON public.marketplace_orders FOR SELECT USING (user_id = auth.uid())';
  END IF;
END $$;
